...
% ── Metadados da versão ─────────────────────────────────────────────────────
\newcommand{\hayversiontag}{v1.1.0}
...
% ─────────────────────────────────────────────────────────---
\section{\texttt{filter\_data(df, col, value)} --- Filtragem de Dados}
\index{filter\_data()}
% ─────────────────────────────────────────────────────────---

Filtra um DataFrame para incluir apenas as linhas onde a coluna especificada é igual ao valor dado.
Esta é uma abordagem simplificada de faceting — filtre os dados manualmente, depois chame \code{hayplot} para cada grupo.

\subsection*{Assinatura}

\begin{lstlisting}[style=inline]
filter_data(df: DataFrame, col: String, value: Float) -> Result<DataFrame, String>
\end{lstlisting}
...
\begin{tabularx}{\textwidth}{@{}lL{2.5cm}X@{}}
  ...
  \textbf{Parâmetro} & \textbf{Tipo} & \textbf{Descrição} \\
  \midrule
  \code{df}      & \code{DataFrame} & O DataFrame a ser filtrado. \\[4pt]
  \code{col}     & \code{String}   & Nome da coluna numérica usada para filtragem. \\[4pt]
  \code{value}   & \code{Float}    & Valor para igualar (tolerância de 1e-9). \\
  \bottomrule
\end{tabularx}

\subsection*{Retorno}

Um novo DataFrame contendo apenas as linhas onde \code{col == value}.

\subsection*{Exemplo}

\begin{lstlisting}[caption={Faceting manual com filter\_data}]
let df1 = gg::filter_data(df, "categoria", 1.0)
let plot1 = gg::hayplot(df1, {"x": "tempo", "y": "valor"})
           |> gg::geom_point("blue", 5.0)
           |> gg::labs("Categoria 1", "Tempo", "Valor")
let svg1 = gg::render_svg(plot1)
\end{lstlisting}

\begin{tipbox}[title={\textbf{Nota:} Faceting manual}]
  Para faceting, filtre os dados usando \code{filter\_data()} para cada grupo e chame \code{hayplot} manualmente.
  As funções \code{facet\_wrap} e \code{render\_facets} foram descontinuadas em favor desta abordagem.
\end{tipbox}

% ─────────────────────────────────────────────────────────---
\section{\texttt{set\_dimensions(plot, width, height)} --- Dimensões do SVG}
\index{set\_dimensions()}
% ─────────────────────────────────────────────────────────---
...
\begin{tabularx}{\textwidth}{@{}lL{2.5cm}X@{}}
  ...
  \code{width}   & \code{Int}    & Largura do SVG em pixels (padrão: 800). \\[4pt]
  \code{height}  & \code{Int}    & Altura do SVG em pixels (padrão: 600). \\
  \bottomrule
\end{tabularx}

\subsection*{Exemplo}

\begin{lstlisting}[caption={Dimensões customizadas}]
let plot = gg::hayplot(df, {"x": "x", "y": "y"})
...
% ─────────────────────────────────────────────────────────---
\section{\texttt{set\_margins(plot, top, bottom, left, right)} --- Margens do Plot}
\index{set\_margins()}
% ─────────────────────────────────────────────────────────---
...
\begin{lstlisting}[style=inline]
set_margins(plot: Dict, top: Int, bottom: Int, left: Int, right: Int) -> Dict
\end{lstlisting}
...
\begin{tabularx}{\textwidth}{@{}lL{2.5cm}X@{}}
  ...
  \midrule
  \code{plot}    & \code{Dict}   & A especificação de plot. \\[4pt]
  \code{top}     & \code{Int}    & Margem superior em pixels (padrão: 20). \\[4pt]
  \code{bottom}  & \code{Int}    & Margem inferior em pixels (padrão: 20). \\[4pt]
  \code{left}    & \code{Int}    & Margem esquerda em pixels (padrão: 20). \\[4pt]
  \code{right}   & \code{Int}    & Margem direita em pixels (padrão: 20). \\
  \bottomrule
\end{tabularx}

\subsection*{Exemplo}

\begin{lstlisting}[caption={Margens customizadas}]
let plot = gg::hayplot(df, {"x": "x", "y": "y"})
...
% ─────────────────────────────────────────────────────────---
\section{\texttt{save\_svg(plot, filename)} --- Salvar SVG}
\index{save\_svg()}
% ─────────────────────────────────────────────────────────---

Renderiza o plot e salva diretamente em um arquivo. Função de conveniência que combina \code{render\_svg()} + \code{write()}.
...
\begin{tabularx}{\textwidth}{@{}lL{2.5cm}X@{}}
  ...
  \midrule
  \code{plot}     & \code{Dict}   & A especificação de plot. \\[4pt]
  \code{filename} & \code{String} & Nome do arquivo para salvar o SVG. \\
  \bottomrule
\end{tabularx}
...
\begin{lstlisting}[caption={Salvamento em um passo}]
let plot = gg::hayplot(df, {"x": "x", "y": "y"})
           |> gg::geom_point("blue", 5.0)
           |> gg::geom_line("red", 2.0)
...
% ─────────────────────────────────────────────────────────---
\section{\texttt{set\_background\_color(plot, color)} --- Cor de Fundo}
\index{set\_background\_color()}
% ─────────────────────────────────────────────────────────---
...
\begin{tabularx}{\textwidth}{@{}lL{2.5cm}X@{}}
  ...
  \midrule
  \code{plot}  & \code{Dict}   & A especificação de plot. \\[4pt]
  \code{color} & \code{String} & Nome da cor ou código hex (ex: \code{"white"}, \code{"\#000000"}). \\
  \bottomrule
\end{tabularx}

\subsection*{Exemplo}

\begin{lstlisting}[caption={Tema escuro}]
let plot = gg::hayplot(df, {"x": "x", "y": "y"})
...
% ─────────────────────────────────────────────────────────---
\section{\texttt{set\_grid(plot, show\_grid)} --- Controle do Grid}
\index{set\_grid()}
% ─────────────────────────────────────────────────────────---

Habilita ou desabilita o grid do plot. O padrão é \code{true} (grid habilitado).
...
\begin{tabularx}{\textwidth}{@{}lL{2.5cm}X@{}}
  ...
  \midrule
  \code{plot}      & \code{Dict}   & A especificação de plot. \\[4pt]
  \code{show\_grid} & \code{Bool}   & \code{true} para habilitar, \code{false} para desabilitar. \\
  \bottomrule
\end{tabularx}

\subsection*{Exemplo}

\begin{lstlisting}[caption={Plot sem grid}]
let plot = gg::hayplot(df, {"x": "x", "y": "y"})
           |> gg::set_grid(false)
           |> gg::geom_point("blue", 5.0)
\end{lstlisting}
...
% ─────────────────────────────────────────────────────────---
\section{\texttt{save\_png(plot, filename)} --- Salvar PNG}
\index{save\_png()}
% ─────────────────────────────────────────────────────────---

Renderiza o plot e salva como arquivo PNG. Requer a feature \code{png} durante a compilação. Função de conveniência que combina renderização PNG + escrita de arquivo.
...
\begin{tabularx}{\textwidth}{@{}lL{2.5cm}X@{}}
  ...
  \midrule
  \code{plot}     & \code{Dict}   & A especificação de plot. \\[4pt]
  \code{filename} & \code{String} & Nome do arquivo para salvar o PNG. \\
  \bottomrule
\end{tabularx}
...
Requer compilação com a feature \code{png}: \code{cargo build --release --features png}. O backend PNG atualmente suporta apenas geometrias básicas (point, line, bar, area).

\subsection*{Exemplo}

\begin{lstlisting}[caption={Salvamento PNG em um passo}]
let plot = gg::hayplot(df, {"x": "x", "y": "y"})
           |> gg::geom_point("blue", 5.0)
           |> gg::geom_line("red", 2.0)
let png = gg::save_png(plot, "output.png")
\end{lstlisting}
...
\section{Faceting Manual}
\index{exemplo!faceting manual}

Visualização de dados agrupados usando filtragem manual de dados:

\begin{lstlisting}[caption={examples/filter\_facet.hay}]
import("sheep-farm/hayplot", as=gg)
...
// Filtrar e renderizar cada grupo manualmente
...
\begin{center}
  ...
  \begin{tabularx}{\textwidth}{@{}lXl@{}}
    ...
    v0.8.0 & \code{filter\_data} --- filtragem de DataFrames                    & $\checkmark$ Conclu\'{i}do \\
    v0.8.0 & \code{facet\_wrap}, \code{render\_facets} --- descontinuados        & $\checkmark$ Substituídos por \code{filter\_data} \\
    v0.9.0 & \code{set\_dimensions} --- tamanho customizável de SVG             & $\checkmark$ Conclu\'{i}do \\
    v0.9.0 & \code{set\_margins} --- margens customizáveis                        & $\checkmark$ Conclu\'{i}do \\
    v0.9.0 & \code{save\_svg} --- render + save em um passo                      & $\checkmark$ Conclu\'{i}do \\
    v1.0.0 & \code{set\_background\_color} --- cor de fundo customizável          & $\checkmark$ Conclu\'{i}do \\
    v1.0.0 & \code{set\_grid} --- controle do grid on/off                          & $\checkmark$ Conclu\'{i}do \\
    v1.1.0 & \code{save\_png} --- exportação PNG (feature flag)                 & $\checkmark$ Conclu\'{i}do \\
    v1.1.0 & Suporte completo de geometrias em PNG                               & Futuro \\
    v1.1.0 & \code{geom\_ribbon} --- bandas de intervalo de confiança           & Futuro \\
    v1.1.0 & \code{geom\_smooth} --- regressão LOESS/linear                    & Futuro \\
    v1.1.0 & \code{theme\_*} --- temas de estilo (dark, minimal, classic)       & Futuro \\
    \bottomrule
  \end{tabularx}
\end{center}
...
\begin{center}
  ...
  \begin{tabularx}{\textwidth}{@{}L{4.8cm}L{4.0cm}X@{}}
    ...
    \code{filter\_data(df, col, value)}           & \code{DataFrame, String, Float}     & Filtra DataFrame onde col == value \\[3pt]
    \code{set\_dimensions(plot, w, h)}             & \code{Dict, Int, Int}               & Define dimensões do SVG (padrão: 800x600) \\[3pt]
    \code{set\_margins(plot, t, b, l, r)}          & \code{Dict, Int, Int, Int, Int}     & Define margens do plot (padrão: 20px) \\[3pt]
    \code{set\_background\_color(plot, color)}     & \code{Dict, String}                 & Define cor de fundo (padrão: white) \\[3pt]
    \code{set\_grid(plot, show\_grid)}               & \code{Dict, Bool}                   & Habilita/desabilita grid (padrão: true) \\[3pt]
    \code{save\_svg(plot, filename)}               & \code{Dict, String}                 & Renderiza e salva SVG em um passo \\[3pt]
    \code{save\_png(plot, filename)}               & \code{Dict, String}                 & Renderiza e salva PNG em um passo (feature: png) \\[3pt]
...
\begin{alertbox}[title={\textbf{Nota:} Faceting descontinuado}]
  As funções \code{facet\_wrap} e \code{render\_facets} foram descontinuadas na v0.8.0.
  Use \code{filter\_data()} para filtrar DataFrames manualmente e chame \code{hayplot} para cada grupo.
\end{alertbox}

\bigskip

\begin{tipbox}[title={\textbf{Nota:} PNG export}]
  A função \code{save\_png} requer compilação com a feature \code{png}: \code{cargo build --release --features png}.
  O backend PNG atualmente suporta apenas geometrias básicas (point, line, bar, area).
\end{tipbox}
